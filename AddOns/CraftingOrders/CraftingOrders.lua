...
local function GetEnchantTargetLink(rec)
    ...
    if rec.targetBag == 255 then
        if rec.targetSlot >= 0 and rec.targetSlot < 19 and GetInventoryItemLink then
            local link = GetInventoryItemLink("player", rec.targetSlot + 1);
            if link then
        ...
        if rec.targetSlot >= 23 and rec.targetSlot <= 38 and GetContainerItemLink then
            local link = GetContainerItemLink(0, rec.targetSlot - 22);
            if link then
                local _, _, itemString = string.find(link, "|H(item:[^|]+)|h");
                return itemString or link;
            end
    ...
    if rec.targetBag and rec.targetBag >= 19 and rec.targetBag <= 22 and GetContainerItemLink then
        local apiBag = rec.targetBag - 18;
        local link = GetContainerItemLink(apiBag, rec.targetSlot + 1);
        if link then
            local _, _, itemString = string.find(link, "|H(item:[^|]+)|h");
            return itemString or link;
        end
...
local function GetCraftingOrderRecipeTooltipLink(rec)
    ...
    -- The Vanilla client and item-tooltip addons such as aux do not accept
    -- spell:<id> in GameTooltip:SetHyperlink.
    return nil;
end
...
local function SetEnchantTargetTooltip(rec, owner, anchor)
    if not rec or rec.targetBag == nil or rec.targetSlot == nil or not owner then
        return false;
    end

    GameTooltip:SetOwner(owner, anchor or "ANCHOR_RIGHT");
    if rec.targetBag == 255 then
        if rec.targetSlot >= 0 and rec.targetSlot < 19 and GameTooltip.SetInventoryItem then
            GameTooltip:SetInventoryItem("player", rec.targetSlot + 1);
            GameTooltip:Show();
            return true;
        end

        if rec.targetSlot >= 23 and rec.targetSlot <= 38 and GameTooltip.SetBagItem then
            GameTooltip:SetBagItem(0, rec.targetSlot - 22);
            GameTooltip:Show();
            return true;
        end
    end

    if rec.targetBag >= 19 and rec.targetBag <= 22 and GameTooltip.SetBagItem then
        GameTooltip:SetBagItem(rec.targetBag - 18, rec.targetSlot + 1);
        GameTooltip:Show();
        return true;
    end
...
local function SetEnchantRecipeTooltip(rec, owner, anchor)
    if not rec or not owner or rec.isEnchantTarget or not State.isEnchantMode or
        not rec.spellId or rec.spellId <= 0 then
        return false;
    end

    GameTooltip:SetOwner(owner, anchor or "ANCHOR_RIGHT");
    GameTooltip:SetText(rec.itemName or "Enchant", 1.0, 0.82, 0.0, 1, 1);
    if rec.enchantDescription and rec.enchantDescription ~= "" and
        rec.enchantDescription ~= rec.itemName then
        GameTooltip:AddLine(rec.enchantDescription, 1.0, 1.0, 1.0, 1);
    end
    GameTooltip:Show();
    return true;
end
...
local function GetRecipeIcon(rec)
    ...
    -- Direct enchants do not have a created item, so GetItemInfo cannot
    -- provide their icon.  The server supplies the spell texture because
    -- the Vanilla client has no GetSpellInfo(spellId) API.
    if rec.spellIcon and rec.spellIcon ~= "" then
        return rec.spellIcon;
    end
...
local function BuildEnchantTargetRecipes(dataStr, baseRecipe)
    ...
    local spellIcon = baseRecipe and baseRecipe.spellIcon or nil;
...
local function BuildRecipesFromString(dataStr)
    ...
    -- spell icon,enchant description.
    local recipes = {};
    ...
    for _, entry in ipairs(SplitEscaped(dataStr, "|")) do
        ...
        if table.getn(hdr) >= 8 then
            ...
            local rec = {
                ...
                spellIcon = UnescapeField(hdr[14] or ""),
                enchantDescription = UnescapeField(hdr[15] or ""),
                materials = {},
            };
...
function CraftingOrderItem_OnEnter(self)
    ...
    if State.selectedRecipeIndex > 0 then
        rec = CraftingOrder_GetRecipeByFilteredIndex(State.selectedRecipeIndex);
    end

    if SetEnchantTargetTooltip(rec, self, "ANCHOR_RIGHT") then
        CursorUpdate(self);
        return;
    end

    if SetEnchantRecipeTooltip(rec, self, "ANCHOR_RIGHT") then
        CursorUpdate(self);
        return;
    end

    if not itemLink and rec then
...
function CraftingOrderSkillButton_OnEnter(self)
    self = self or this;
    if not self then
        return;
    end
    ...
    local row = CraftingOrder_GetRowByFilteredIndex(self:GetID());
    if row and row.recipe and row.recipe.isEnchantTarget then
        SetEnchantTargetTooltip(row.recipe, self, "ANCHOR_RIGHT");
    end
end

function CraftingOrderSkillButton_OnLeave(self)
    self = self or this;
...
function CraftingOrderCancelButton_OnClick()
    if State.selectingEnchantTarget then
        ...
        State.selectedEnchantSpellId = nil;
        State.selectedEnchantSpellName = nil;
        State.recipes = State.baseEnchantRecipes or {};
        State.baseEnchantRecipes = nil;
...
-- us the same callback shape as the working CraftingStorage addon while the
