...
namespace
{
    uint32 GetPlayerLocale(Player* player)
    {
        if (player && player->GetSession())
            return uint32(player->GetSession()->GetSessionDbcLocale());
        return LOCALE_enUS;
    }

    std::string FirstSpellName(SpellEntry const* spellInfo, uint32 preferredLocale)
    {
        if (!spellInfo)
            return std::string();

        if (preferredLocale < MAX_DBC_LOCALE && !spellInfo->SpellName[preferredLocale].empty())
            return spellInfo->SpellName[preferredLocale];
        if (!spellInfo->SpellName[LOCALE_enUS].empty())
            return spellInfo->SpellName[LOCALE_enUS];
        for (std::string const& name : spellInfo->SpellName)
        {
            if (!name.empty())
                return name;
        }
    ...
    std::string FirstEnchantDescription(uint32 enchantId, uint32 preferredLocale)
    {
        ...
        if (preferredLocale < MAX_DBC_LOCALE && enchant->description[preferredLocale] &&
            enchant->description[preferredLocale][0])
            return enchant->description[preferredLocale];
        if (enchant->description[LOCALE_enUS] && enchant->description[LOCALE_enUS][0])
            return enchant->description[LOCALE_enUS];
        for (uint32 locale = 0; locale < MAX_DBC_LOCALE; ++locale)
        {
            if (enchant->description[locale] && enchant->description[locale][0])
                return enchant->description[locale];
        }
        return std::string();
    }

    std::string GetRecipeDisplayName(Player* player, RecipeData const& recipe,
        ItemPrototype const* createdItem)
    {
        if (createdItem && !createdItem->Name1.empty())
            return createdItem->Name1;

        uint32 const locale = GetPlayerLocale(player);

        std::string name = FirstSpellName(sSpellMgr.GetSpellEntry(recipe.spellId), locale);
        if (name.empty())
            name = recipe.displayName;
        if (name.empty() && recipe.isEnchantSpell)
            name = FirstEnchantDescription(recipe.enchantId, locale);
        if (name.empty())
            name = recipe.isEnchantSpell ? "Unknown Enchant" : "Unknown Recipe";
        return name;
    }
    ...
    std::string const& filter, uint32 tier, uint32 maxSkillRank) const
{
    ...
    for (RecipeData const& recipe : recipes)
    {
        ...
        else if (!filter.empty() && filter != "MAKEABLE")
        {
            ...
            std::string itemName = GetRecipeDisplayName(player, recipe, itemTmpl);
        ...
        if (recipe.isEnchantSpell)
        {
            SpellEntry const* spellInfo = sSpellMgr.GetSpellEntry(recipe.spellId);
            if (spellInfo)
                spellIcon = spellInfo->GetIcon(spellInfo->SpellIconID);
            enchantDescription = FirstEnchantDescription(recipe.enchantId, GetPlayerLocale(player));
        }
        ...
        ss << recipe.spellId << "," << recipe.createdItemId << ","
           ...
           << remaining << ","
           << CraftingOrdersDomain::EscapeField(spellIcon) << ","
           << CraftingOrdersDomain::EscapeField(enchantDescription);
        for (CraftMaterial const& mat : recipe.materials)
        {
