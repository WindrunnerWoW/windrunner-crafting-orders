...
{
    // A reload must not retain sessions created under the previous config.
    // In particular, a disable/re-enable cycle must require a fresh NPC
    // interaction.
    _sessions.clear();
    _loaded = false;
...
bool CraftingOrders::MakeRecipeData(uint32 spellId, uint32 skillLine, uint32 skillRank, uint32 reqLevel, RecipeData& rd) const
{
    ...
    // Some direct-enchant spell rows have no usable spell display name on
    // this 1.12 data set.  The enchantment DBC is the authoritative fallback
    // for the name shown by the normal enchanting UI.
    if (name.empty() && isEnchant && enchantId)
    {
        SpellItemEnchantmentEntry const* enchant = sSpellItemEnchantmentStore.LookupEntry(enchantId);
        if (enchant)
        {
            for (uint32 locale = 0; locale < MAX_DBC_LOCALE && name.empty(); ++locale)
            {
                if (enchant->description[locale] && enchant->description[locale][0])
                    name = enchant->description[locale];
            }
        }
    }
...
void CraftingOrders::LoadRecipes()
{
    ...
    uint32 loaded = 0;
    uint32 skipped = 0;
    // Turtle's Jewelcrafting trainers use npc_trainer_template. Trainer
    // discovery already reads both trainer sources, so do the same for the
    // recipe catalogue.
    std::unique_ptr<QueryResult> result(WorldDatabase.PQuery(
        "SELECT DISTINCT spell, reqskill, reqskillvalue, reqlevel FROM ("
        "SELECT spell, reqskill, reqskillvalue, reqlevel FROM npc_trainer WHERE reqskill IN (%s) "
        "UNION ALL "
        "SELECT spell, reqskill, reqskillvalue, reqlevel FROM npc_trainer_template WHERE reqskill IN (%s)"
        ") AS trainer_recipes ORDER BY reqskill, reqskillvalue, spell",
        profCsv.c_str(), profCsv.c_str()));
    if (!result)
        sLog.outError("[mod-crafting-orders] No trainer rows found for supported professions.");
    else
    {
        do
        {
